    ...
    while(1){
       ...
       while(1){
        ...
        sum = 0;
         ...
         char *psp = strtok(line, " ");
         while(psp != NULL)
         {
             spline[toks] = strdup(psp);
             sum = sum + strtol(spline[toks], &ptr, 10);
             toks++;
             psp = strtok(NULL, " ");
         }

         strcat(line, " SUM=");
         sprintf(append,"%ld",sum); 
         strcat(line, append);
         //strcat(line, (const char*)sum);
         // send the echo line to client 
         n = write(client_sock, line, MAX);

         printf("server: wrote n=%d bytes; ECHO=[%s];  sum=%ld\n", n, line, sum);
       }
    }
}
